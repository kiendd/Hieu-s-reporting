...
**LLM config:** `llm_extractor.configure()` reads `OPENAI_API_KEY` / `OPENAI_BASE_URL` / `LLM_MODEL` / `LLM_STRUCTURED_OUTPUTS` from env (defaults: `https://api.openai.com/v1` / `gpt-5.4-mini` / on). CLI flags `--llm-base-url` / `--llm-model` / `--llm-structured-outputs` / `--no-llm-structured-outputs` override env. Streamlit stores all four in browser `localStorage` under `fpt_llm_base_url` / `fpt_llm_model` / `fpt_llm_api_key` / `fpt_llm_structured_outputs` and NEVER writes the API key to `config.json` — same policy as the FPT chat token.
